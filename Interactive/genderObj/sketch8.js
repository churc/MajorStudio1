...
//=======  group sort by artist by object type
    groupedByName = collate(p.tableA,12);  ////name, Artist Alpha Sort with array of artworks by artist
    // console.log(groupedByName);

    //=======  group sort by objectbegin date
    groupedByYear = collate(p.tableA,17);  ////name, Artist Alpha Sort with array of artworks by artist
    console.log(groupedByYear);
...
//=================== classification object type
   var groupedByType = collate(p.tableA,22);  ////classification
    console.log(groupedByType);
    // groupByObjType.push(groupedByType);
...
////iterate through object by keys and SPLITS object into 105 objects 
  ...
  p.push();
  var groupedByYear = collate(p.tableA,17);  ////name, Artist Alpha Sort with array of artworks by artist
...
for(row =0; row<count; row++){
    p.stroke(p.color(179,118,244));
    p.beginShape();
    for(col=3; col<countC; col++){
      // val = p.float(p.int(p.table.getString(row, 22)));
      val = p.float(p.int(p.table.getString(row, col)));
