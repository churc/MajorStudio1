...
barTotalsSort = barTotals;
      ...
      for (var i = barTotalsSort.length-1; i >= 0; i--) {
          ...
          bars.push({x:160 + (i * 15), y:$(window).height()/2, width: 15, height: barTotals[i]['f']/4, name:barTotals[i].name, maleHeight: barTotals[i]['m']/2});
          ...
          // p.fill(255,0,0);
          p.textSize(14);
           p.text(barTotalsSort[i].name+barTotalsSort[i].total, 5+7, 105+(i * 10));
...
p.draw = function(){
  ...
  for (var i = bars.length - 1; i >= 0; i--) {
   ...
   if ((p.mouseX >= bars[i].x) && (p.mouseX <= bars[i].x + 15) && (p.mouseY >= (bars[i].y-bars[i].height) && p.mouseY <= (bars[i].y +bars[i].maleHeight))){
...
p.header = function(){
    p.push();
      ...
      p.fill(77,77,77);
      p.textSize(44);
      p.text("Gender and Medium: The Met Modern & Contemporary Art", 50, 48);
      // p.textSize(40);
...
p.push();
  ...
  p.scale(0.75);
    ...
    p.translate(2260,1845);
 // p.translate(1900,1800);
    ...
    for(var i=lowerLimit; i<upperLimit; i+=100){
      var x = margin;
  ...
  p.pop();
    ...
    // p.rotate(p.radians(90));
        ...
        p.fill(105,105,105);
        p.textSize(30);
        p.text("male", 1200, 300);
        p.text("female", 40, 300);
        p.fill(92,242,145,150);
        // p.ellipse(500, margin, 25, 25);
        // p.fill(179,118,244,130);
        // p.ellipse(335, margin, 25,25);
