...
for (var i = barTotalsSort.length-1; i >= 0; i--) {
	...
	var x = p.map(i, barTotalsSort.length-1, 0, w-margin*2, 0-margin*2);
...
//      p.rect(160 + (i * 10), $(window).height()/2.12, 10, barTotals[i]['m']/2);
	 ...
	 rect1 = p.rect((barTotals[barTotals.length - 1]['f'])/2 - (margin*2) + (i * 10), (barTotals[barTotals.length - 1]['f'])/2 + (margin), 10, barTotals[i]['m']/2);
	
	console.log(barTotals[barTotals.length - 1]['f']);
	
      ////1980 screen
//           p.rect(160 + (i * 10), $(window).height()/2.87, 10, barTotals[i]['m']/2);

	var y = p.map(i,barTotalsSort.length-1, 0, w-margin*2, 0-margin*2);
...
//          var bar = p.rect(160 + (i * 10), $(window).height()/2.12, 10, (-1) * barTotals[i]['f']/2);
	...
	var bar = p.rect((barTotals[barTotals.length - 1]['f'])/2-(margin*2) + (i * 10), (barTotals[barTotals.length - 1]['f'])/2 + (margin), 10, (-1) * barTotals[i]['f']/2);
...
///draw axis & number labels on axis 
    for(var i=lowerLimit; i<upperLimit; i+=50){

		var x = h-(margin*2);
		
		var y = p.map(i,upperLimit, lowerLimit, lowerLimit+(w*3.4)+margin*2, upperLimit+(w*3.4)+margin*2); 
